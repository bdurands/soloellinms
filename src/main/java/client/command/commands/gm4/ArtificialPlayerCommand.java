...
public class ArtificialPlayerCommand extends Command {
    ...
    public void execute(Client c, String[] params) {
        ...
        if (first.equals("spawntype")) {
            ExecutorServiceManager.getExecutorService().execute(() -> handleSpawnBotType(params, c));
            return;
        }
...
//                break;
    ...
    private static void handleSpawnBotType(String[] params, Client c) {
        if (params.length < 2) {
            player.yellowMessage("Usage: !bot spawntype <type> [count]");
            player.yellowMessage("Types: fm, selling, buying, nx, gacha, henesys, gzh, bj, dg, opq, jq, dice, tutorial, social");
            return;
        }

        String typeStr = params[1].toLowerCase();
        BotTypeManager.BotType type = null;
        switch (typeStr) {
            case "fm": type = BotTypeManager.BotType.FM_BOT; break;
            case "selling": type = BotTypeManager.BotType.SELLING_MERCHANT_BOT; break;
            case "buying": type = BotTypeManager.BotType.BUYING_MERCHANT_BOT; break;
            case "nx": type = BotTypeManager.BotType.NX_MERCHANT_BOT; break;
            case "gacha": type = BotTypeManager.BotType.GACHA_BOT; break;
            case "henesys": type = BotTypeManager.BotType.HENESYS_BOT; break;
            case "gzh": type = BotTypeManager.BotType.GAME_ZONE_HOST_BOT; break;
            case "bj": type = BotTypeManager.BotType.BLACKJACK_DEALER; break;
            case "dg": type = BotTypeManager.BotType.DROP_GAME_BOT; break;
            case "opq": type = BotTypeManager.BotType.OPQ_BOT; break;
            case "jq": type = BotTypeManager.BotType.HENESYS_JQ_BOT; break;
            case "dice": type = BotTypeManager.BotType.DICE_BOT; break;
            case "tutorial": type = BotTypeManager.BotType.TUTORIAL_BOT; break;
            case "social": type = BotTypeManager.BotType.SOCIAL_BOT; break;
            default:
                player.yellowMessage("Unknown type: " + typeStr);
                return;
        }
        ...
        for (int i = 0; i < count; i++) {
            // Se usa el constructor de creación con clase 1 (warrior base), nivel 10 a 20 aleatorio, pero el tipo de bot lo redefinirá visualmente
            int botId = BotGeneration.createBot(pos, map, 1, 10, 20);
            Character fakechar = BotHelpers.getCharFromChannelStorage(botId);
            if (fakechar != null) {
                type.createAndSetBot(fakechar);
                BotTypeManager.manuallyStartBot(fakechar);
                spawned.add(botId);
            }
        }
        player.yellowMessage("Spawned " + count + " " + type.name() + " bot(s): cids " + spawned);
    }
